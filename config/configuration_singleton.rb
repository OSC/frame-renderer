...
class ConfigurationSingleton
  ...
  def config_root
    Pathname.new(ENV['OOD_APP_CONFIG_ROOT'] || '/etc/ood/config/apps/frame-renderer')
  end
  ...
  #
  # Doing this in two separate loads means OOD_APP_CONFIG_ROOT can be specified
  # in the .env.local file, which will specify where to look for the /etc dotenv
  # files. The default for OOD_APP_CONFIG_ROOT is 
  # /etc/ood/config/apps/frame-renderer and both .env and .env.production will 
  ...
  FALSE_VALUES = [
    nil, false, '', 0, '0', 'f', 'F', 'false', 'FALSE', 'off', 'OFF', 'no', 'NO'
  ].freeze
